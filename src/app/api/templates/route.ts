...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.preview_image_base64) {
      ...
      previewImageUrl = `/uploads/${fileName}`; // URL publique
    }
    ...
    const response = NextResponse.json({ data: newTemplate[0] }, { status: 201 });
    return addCORSHeaders(response);
  } catch (error: any) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if ((updateResult as any).affectedRows === 0) throw new Error('Template non trouvé');

    const [updated] = await pool.execute('SELECT * FROM templates WHERE id = ?', [body.id]);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if ((result as any).affectedRows === 0) throw new Error('Template non trouvé');
